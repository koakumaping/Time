...
local function refreshZoneTiny(wgt)
  ...
  local timerInfo = string.format("%s", t1.name)
  lcd.drawText(wgt.zone.x, wgt.zone.y, timerInfo, SMLSIZE + CUSTOM_COLOR)
  lcd.drawNumber(wgt.zone.x, wgt.zone.y+14, t1.value / 60, MIDSIZE + CUSTOM_COLOR)
end
...
return { name="Time", options=options, create=create, update=update, background=background, refresh=refresh }
